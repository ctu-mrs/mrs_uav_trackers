/* Produced by CVXGEN, 2018-03-26 08:37:23 -0400.  */
...
float randn_internal_xy(long *idum, int reset) {
  ...
  if (iset==0) {
    do {
      v1 = 2.0*ran1d_xy(idum, reset)-1.0;
      v2 = 2.0*ran1d_xy(idum, reset)-1.0;
      rsq = v1*v1+v2*v2;
...
double randn_xy(void) {
  return randn_internal_xy(&global_seed, 0);
}
void reset_rand_xy(void) {
  srand(15);
  global_seed = 1;
  randn_internal_xy(&global_seed, 1);
}
